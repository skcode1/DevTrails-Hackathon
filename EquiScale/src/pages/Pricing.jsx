import { Calculator, History, Zap, ShieldCheck, Info, ArrowRight, Activity, Cpu } from "lucide-react";
...
export default function Pricing() {
  const { result, calculatePremium, loading } = usePricing();
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4 lg:p-10 space-y-10 animate-in fade-in duration-700">
      ...
      <header className="relative flex flex-col md:flex-row md:items-center justify-between gap-6 pb-10 border-b border-slate-200/60">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 text-sm font-medium max-w-xl">
            {PRICING_CONFIG.LABELS.headerDesc}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-4 space-y-8">
          <section className="group relative bg-white rounded-[2rem] border border-slate-200 shadow-xl shadow-slate-200/40 overflow-hidden transition-all hover:shadow-2xl">
            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-white rounded-lg shadow-sm border border-slate-200">
                  <History className="w-4 h-4 text-slate-600" />
                </div>
                <h3 className="font-black !text-black text-xs uppercase tracking-widest">Data Ingestion</h3>
              </div>
              <Info className="w-4 h-4 text-slate-300 hover:text-cyan-500 transition-colors cursor-help" />
            </div>
            
            <div className="p-8">
              <EarningsUploadForm onSubmit={calculatePremium} isLoading={loading} />
              
              <div className="mt-8 p-4 bg-slate-900 rounded-2xl relative overflow-hidden">
          ...
          {/* PERFORMANCE CARD */}
          <div className="p-8 bg-slate-950 rounded-[2rem] text-white shadow-2xl relative overflow-hidden animate-slow-pan bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-950">
            <Cpu className="absolute -right-4 -top-4 w-32 h-32 text-white/[0.03]" />
            <div className="relative z-10 space-y-6">
              <div>
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400 mb-2">Engine State</h4>
        ...
        {/* RIGHT COLUMN: THE ANALYTICS COMMAND */}
            ...
            <div className="space-y-10 animate-in zoom-in-95 duration-500">
              ...
              <section className="bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl overflow-hidden">
                <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-white">
                  <div>
                    <h3 className="text-xl font-black !text-black italic tracking-tighter uppercase">Markov Transition Matrix</h3>
                    <p className="text-[10px] font-bold text-slate-400 tracking-widest mt-1 uppercase">
                      {PRICING_CONFIG.LABELS.markovNote}
                    </p>
                  </div>
...
function LoadingState() {
  return (
    <div className="h-full min-h-[500px] bg-white rounded-[3rem] border border-slate-200 flex flex-col items-center justify-center p-12 shadow-inner">
      ...
      <h3 className="text-2xl font-black !text-black italic uppercase tracking-tighter">Solving Markov Chain</h3>
      <p className="text-slate-400 text-[10px] mt-4 font-black font-mono text-center max-w-xs uppercase tracking-[0.2em]">
        {/* RECTIFIED: Unicode π₀ and Pⁿ */}
...
function EmptyStatePlaceholder() {
  return (
    <div className="h-full min-h-[500px] border-2 border-dashed border-slate-200 rounded-[3rem] flex flex-col items-center justify-center p-12 text-center group transition-all hover:border-cyan-200 hover:bg-cyan-50/10">
      ...
      <h3 className="text-3xl font-black !text-black italic uppercase tracking-tighter">Awaiting Actuarial Data</h3>
      <p className="text-slate-500 text-sm max-w-sm mt-4 leading-relaxed font-medium">
        Inject the rider's risk history into the forge to initialize the 5-layer pricing sequence.
      </p>
