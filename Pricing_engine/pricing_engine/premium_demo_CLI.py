...
# ─────────────────────────────────────────────────────────────────────────────
# Layer 2 — Markov: week-averaged distribution
# ─────────────────────────────────────────────────────────────────────────────

def markov_pi_bar(P: np.ndarray, pi0: np.ndarray, n: int = 5) -> np.ndarray:
    """
    Week-averaged Markov distribution via iteration.
    pi_bar = (1/n) * sum_{t=1}^{n} pi_0 * P^t
    Safe for any P including near-singular matrices.
    """
    ...
    for _ in range(n):
        pi     = pi @ P
        total += pi
    # Normalise — dividing by sum rather than n corrects floating point drift
    return total / total.sum()


def stationary_distribution(P: np.ndarray) -> np.ndarray:
    """Solve pi_inf * P = pi_inf via eigendecomposition."""
...
def fraud_check(
    ...
    """
    # Step 1: Iglewicz-Hoaglin modified Z-score
    L_med = float(np.average(L_ins, weights=pi_bar))
    MAD   = float(np.average(np.abs(L_ins - L_med), weights=pi_bar))

    # When pi_bar is concentrated in s0, MAD collapses to 0.
    # Fall back to weighted standard deviation as the scale denominator.
    if MAD < 1e-6:
    ...
    # Step 2: Markov likelihood ratio
...
if __name__ == "__main__":
    ...
    run_demo(
        ...
        models         = models,
        claim_amount   = 4,       # realistic s1 claim — within L_ins[1] = ~5.55
        claimed_state  = 1,
    )
